// script.js
// STEP 1: Read user from URL
const params = new URLSearchParams(window.location.search);
const incomingUserId = params.get('user_id');
const incomingUserName = params.get('name');

// STEP 2: If URL has valid user, override localStorage
if (incomingUserId && incomingUserName) {
...
// STEP 3: Now use localStorage ALWAYS
...
document.getElementById('logoutBtn').onclick = () => {
  // Do NOT remove chat user
  window.location.href = 'http://localhost:4000'; // Redirect back to CampusWire
};
